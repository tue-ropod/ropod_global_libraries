...
static int user_apc(int a, int b)
{
    if (rtmGetErrorStatus(RT_MDL) == NULL &&
        ...
        if (rtmGetStopRequested(RT_MDL)) return 1;
...
#else
    printf("Motion target v1.4 SOEM, M.J.G. v.d. Molengraft & K.J. Meessen, 2012\n");
#endif
    /* postpone the statistics calculation of the timer to avoid strange number caused by initialization */
    rtc_postpone_statistics_calculation((unsigned int)Freq*1);
